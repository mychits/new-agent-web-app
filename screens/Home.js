...
const HelpModal = ({ visible, onClose }) => {
  ...
  const handleCallUs = () =>
    Linking.openURL(`tel:9483900777`).catch((err) => console.error(err));
...
const GridCard = ({ item, onPress, index }) => {
  ...
  const config = ICON_CONFIG[item.id] || {
    name: "help",
    color: COLORS.primary,
  };
  ...
  return (
    <Animated.View
      style={[{ transform: [{ scale: scaleAnim }] }, styles.gridItemWrapper]}
    >
      <TouchableOpacity
        onPress={onPress}
        activeOpacity={1}
...
const WideCard = ({ item, onPress, index }) => {
  ...
  return (
    <Animated.View
      style={[
        { transform: [{ translateX: slideAnim }], opacity },
    ...
    >
      <TouchableOpacity onPress={onPress} activeOpacity={0.9}>
        <LinearGradient
          ...
          style={[
            styles.wideCard,
            item.variant === "overview" && styles.overviewLayout,
          ]}
          start={{ x: 0, y: 0 }}
          end={{ x: 1, y: 1 }}
        >
          ...
          {item.variant === "overview" ? (
            <>
              ...
              <View style={styles.overviewTextContainer}>
                <Text style={[styles.wideTitle, styles.overviewTitleText]}>
                  My Overview
                </Text>
                <Text
                  style={[
...
const Home = ({ route, navigation }) => {
        ...
        try {
          ...
          if (response.data) {
            ...
            if (response.data?.designation_id?.permission) {
              setModifyPayment(
                response.data.designation_id.permission.modify_payments ===
                  "true",
              );
            }
          }
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onMoveShouldSetPanResponder: (_, gestureState) =>
        isSideMenuVisible && gestureState.dx < -10,
      onPanResponderMove: (_, gestureState) => {
        if (gestureState.dx < 0)
          slideAnim.setValue(Math.max(gestureState.dx, -width));
      },
      onPanResponderRelease: (_, gestureState) => {
        if (gestureState.dx < -width / 2) closeSideMenu();
        else openSideMenu();
      },
    }),
  ).current;
  ...
  const cardsData = [
    ...
    {
      id: "approvals",
      name: "Loan Applications", // NEW ITEM ADDED HERE
  ...
  const topWideCards = cardsData.filter(
    (item) =>
      item.isFullWidth && item.id !== "rewards" && item.id !== "starPoints",
  );
  const rewardsStarPointsCards = cardsData.filter(
    (item) => item.id === "rewards" || item.id === "starPoints",
  );
        ...
        <View style={styles.footerSection}>
          ...
          <View style={styles.supportRow}>
            <TouchableOpacity
              ...
              onPress={() => Linking.openURL("tel:9483900777")}
            >
            ...
            <TouchableOpacity
              style={styles.supportBtn}
              onPress={() =>
                Linking.openURL("whatsapp://send?phone=919483900777")
              }
            >
              <View style={styles.supportCircle}>
                <Ionicons
...
const styles = StyleSheet.create({
  ...
  wideTextContainer: { flex: 1 },
  overviewTextContainer: { alignItems: "center" },
  wideTitle: { color: "#fff", fontSize: 17, fontWeight: "bold" },
  overviewTitleText: {
    fontSize: 18,
  ...
  qrTitle: { color: "#fff", fontSize: 19, fontWeight: "bold" },
  qrBox: {
    backgroundColor: "#fff",
    padding: 12,
    borderRadius: 20,
  ...
  helpScroll: { flex: 1, paddingHorizontal: 25 },
  modalMenuItem: {
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "center",
  ...
  menuHeader: { padding: 30, paddingTop: 80, borderBottomLeftRadius: 35 },
  closeMenuBtn: {
    position: "absolute",
    top: 50,
    right: 20,
  ...
  menuScroll: { flex: 1, paddingTop: 30 },
  menuItem: {
    flexDirection: "row",
    alignItems: "center",
    paddingVertical: 16,
  ...
  menuIcon: { marginRight: 20, width: 26 },
  menuText: { fontSize: 17, color: "#444", fontWeight: "600" },
  menuDivider: {
    height: 1,
    backgroundColor: "#eee",
    marginVertical: 15,
