...
const Enrollment = ({ route, navigation }) => {
  ...
  // Normalize incoming filter to internal key
  const normalizeFilter = (filter) => {
    if (filter === "New Groups") return "NewGroups";
    if (filter === "Ongoing Groups") return "OngoingGroups";
    if (filter === "All Groups") return "AllGroups";
    return "AllGroups";
  };

  const initialGroupFilter = normalizeFilter(groupFilter || "All Groups");
  const [selectedGroup, setSelectedGroup] = useState(initialGroupFilter);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState(null);
  const [enrollmentModalVisible, setEnrollmentModalVisible] = useState(false);
  ...
  const fetchGroups = async () => {
    ...
    let endpoint = `${baseUrl}/group/get-group/`; // fallback

    if (selectedGroup === "AllGroups") {
      endpoint = `${baseUrl}/group/get-group-by-filter/AllGroups`;
  ...
  // Sync route param filter to internal state (on mount or change)
  useEffect(() => {
    if (groupFilter) {
      const normalized = normalizeFilter(groupFilter);
  ...
  // Determine color key based on group properties
  const getCustomCardColorKey = (card) => {
    const members = typeof card.group_members === 'number' 
      ? card.group_members 
      : parseInt(card.group_members, 10) || 0;
    const value = typeof card.group_value === 'number' 
      ? card.group_value 
      : parseFloat(card.group_value) || 0;
    const performanceStatus = card.performance_status;
    const category = card.category;

    if (category) {
      const categoryKey = category.replace(/\s+/g, '_').toLowerCase();
      if (groupColors[categoryKey]) return categoryKey;
    }
  ...
  const handleEnrollment = (card) => {
    const selectedGroupId = card._id;
    if (selectedGroupId) {
      navigation.navigate("EnrollForm", { groupId: selectedGroupId, user });
    } else {
      setModalMessage("Error: Could not retrieve group ID.");
      setEnrollmentModalVisible(true);
    }
  ...
  const CardContent = ({ card, colors, isSelected, vacantSeats }) => {
    const vacantSeatCount = vacantSeats[card._id] !== undefined ? vacantSeats[card._id] : "Loading...";
    const isVacant = typeof vacantSeatCount === 'number' && vacantSeatCount > 0;
    ...
    return (
      <View style={styles.cardContent}>
        ...
        <View style={styles.detailsRow}>
          ...
          <View style={styles.detailItemRight}>
            <Text style={[styles.detailLabel, { color: '#263238' }]}>Vacant Seats</Text>
            <Text style={[styles.detailValue, { color: '#da8201' }, isVacant && styles.vacantSeatsHighlight]}>
              {vacantSeatCount}
            </Text>
          </View>
        </View>
  ...
  const renderCards = () => {
        ...
        <View style={styles.emptyStateContainer}>
          ...
          <Text style={styles.noGroupsText}>
            {selectedGroup === "NewGroups"
              ...
              : selectedGroup === "OngoingGroups"
                ? "No ongoing groups found. Check back later!"
                : "It looks like there are no groups that match your current filter. Try changing the filter or check back later!"}
          </Text>
        </View>
      );
    }
    ...
    return cardsData.map((card) => {
      const customColorKey = getCustomCardColorKey(card);
      const colors = groupColors[customColorKey] || groupColors.default;
      const isSelected = selectedCardIndex === card._id;
  ...
  // Map internal key to display label
  const getDisplayLabel = (key) => {
    if (key === "AllGroups") return "All Groups";
    if (key === "NewGroups") return "New Groups";
    if (key === "OngoingGroups") return "Ongoing Groups";
    return key;
  };
        ...
        <View style={styles.mainContentWrapper}>
          <View style={styles.innerContentArea}>
            <View style={styles.filterContainer}>
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipsScrollContainer}>
                <View style={styles.chipsContainer}>
                  {["AllGroups", "NewGroups", "OngoingGroups"].map((filterKey) => (
                    <TouchableOpacity
                      key={filterKey}
                      style={[
                        styles.chip,
                        selectedGroup === filterKey && styles.selectedChip,
                      ]}
                      onPress={() => setSelectedGroup(filterKey)}
                    >
                      <Ionicons
                        name={
                          filterKey === "AllGroups" ? "grid" :
                          filterKey === "NewGroups" ? "sparkles" : "hourglass"
                        }
                        size={16}
                        color={selectedGroup === filterKey ? '#fff' : '#666'}
                        style={styles.chipIcon}
                      />
                      <Text style={[styles.chipText, selectedGroup === filterKey && styles.selectedChipText]}>
                        {getDisplayLabel(filterKey)}
                      </Text>
                    </TouchableOpacity>
                  ))}
                </View>
...
const headerStyles = StyleSheet.create({
 headerContainer: {
    flexDirection: 'row',
    alignItems: 'center',
    // Removed paddingHorizontal: 15 here
    paddingVertical: 10,
    paddingTop: 43,
...
backButton: {
    padding: 5,
    marginRight: 10,
    marginLeft: 15, // Added left margin for screen padding
},
headerImage: {
    width: 150,
    height: 40,
    resizeMode: 'contain',
    marginLeft: 'auto', // Keeps it pushed right
    marginRight: 25, // Added right margin for screen padding
},
});
