...
const PigmePrint = ({ route }) => {
  const { customer_name, phone_number, agent_name, cus_id, actual_pigme_id, pigme_amount, pay_date, amount, pay_type, transaction_id, custom_pigme_id, receipt_no, isPigmePayment, user } = route.params;
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (Array.isArray(response.data)) { setTotalPaidAmount(response?.data?.[0]?.totalPaidAmount); }
      } catch (error) { setTotalPaidAmount("0"); } finally { setLoading(false); }
    })();
  }, []);

  const handleConnect = async () => {
    setIsConnecting(true);
    try { await blePrinter.scanAndConnect(() => { setIsConnected(true); setIsConnecting(false); Alert.alert("Success", "Connected to printer!"); }); } 
    catch (error) { setIsConnecting(false); Alert.alert("Connection Error", "Failed to connect."); }
  };
  ...
  const handleShareWhatsApp = async () => {
    ...
    } catch (error) { Alert.alert("Error", "Failed to share image."); }
  };
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.white }}>
        <View style={{ marginHorizontal: 22, marginTop: 12, flex: 1 }}>
          ...
          <View ref={viewRef} collapsable={false} style={{ padding: 12, backgroundColor: "#f0eeee", borderRadius: 8, marginTop: 5, flexGrow: 1 }}>
            ...
            <Text style={{ fontWeight: "bold", fontSize: 13, textAlign: "center", marginBottom: 10 }}>{isPigmePayment ? "Pigmy Receipt" : "Receipt"}</Text>
            ...
            <Text style={{ ...styles.textStyle, marginVertical: 5 }} />
            <Text style={[styles.textStyle, { fontSize: 14, fontWeight: "bold" }]}>{isPigmePayment ? `Pigmy ID: ${custom_pigme_id || "N/A"}` : `Group: 000 Error 000`}</Text>
            <View style={{ ...styles.amountBox, marginVertical: 10 }}><Text style={styles.amountText}>Received Amount | Rs.{amount || 0}</Text></View>
...
const styles = StyleSheet.create({
  ...
  amountText: { fontSize: 16, fontWeight: "bold" }, buttonRow: { flexDirection: "row", justifyContent: "space-between", marginTop: 18 },
