...
const LoanPayments = ({ route, navigation }) => { 
  ...
  const fetchCustomers = useCallback(async (isRefresh = false) => {
    try {
      ...
      if (response?.data && Array.isArray(response.data)) {
        ...
        const loanPayments = response.data.filter((item) => {
          return (
            item?.pay_for === "Loan" ||
            item?.payment_type === "Loan Payment"
          );
        });

        console.log("Loan payments filtered:", loanPayments.length);
        setCustomers(loanPayments);

        const uniqueUsers = loanPayments.reduce((acc, item) => {
  ...
  }, [user?.userId, selectedDate, selectedCustomer, selectedloanId, selectedPaymentMode]);
  ...
  const filteredCustomers = Array.isArray(customers)
    ? customers.filter((customer) => {
        ...
        return nameMatch && customerMatch && loanMatch && paymentModeMatch;
      })
    : [];
    ...
    switch (selectedFilter) {
      ...
      case 'loan':
        ...
        const filteredLoans = uniqueloanIds.filter((loanId) =>
          loanId?.toString().toLowerCase().includes(loanSearch.toLowerCase())
        );
        ...
        return (
          <View style={styles.pickerWrapper}>
            ...
            <ScrollView style={styles.pickerScrollView}>
              ...
              >
                <Text style={styles.pickerItemText}>All Loan Accounts</Text>
              </TouchableOpacity>
              ...
              {filteredLoans.map((loanId) => (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    updateFilterValue('loan', loanId);
                    setLoanSearch('');
                    closePicker();
                  }}
                >
                  <Text style={styles.pickerItemText}>Loan ID: {loanId}</Text>
                </TouchableOpacity>
              ))}

              {filteredLoans.length === 0 && (
                <View style={styles.noResultsContainer}>
                  <Text style={styles.noResultsText}>No loan IDs found</Text>
                </View>
              )}
            </ScrollView>
  ...
  const printPDF = async () => {
    const filteredCustomersForPrint = filteredCustomers
      .map((customer, index) => `
        <tr>
          <td>${index + 1}</td>
          <td>${customer?.loan?.loan_id || "N/A"}</td>
