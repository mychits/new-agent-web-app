...
`;
  ...
  const printHtmlReceipt = async (widthMM) => {
    ...
    const htmlContent = `
      ...
      <head>
        <style>
          @page { size: ${size} auto; margin: 0 0 0 4mm; }
          body { font-family: Arial, sans-serif; font-size: 14px; margin: 0; padding: 0; width: ${size}; }
          .receipt { padding: 5mm; }
      ...
      <body>
        <div class="receipt">
          <div class="header"><h3 style="margin-bottom: 5px;">MY CHITS</h3></div>
          ...
          <p><span style="font-weight: bold;">Loan Amount:</span> ${loanAmount || "N/A"} <br/>
            <span style="font-weight: bold;">Receipt No:</span> ${receipt_no || "N/A"} <br/>
            <span style="font-weight: bold;">Date:</span> ${formatDate(pay_date)}</p>
          <div style="margin: 10px 0 0 0;"></div>
          <p><span style="font-weight: bold;">Name:</span> ${customer_name || "N/A"} <br/>
            <span style="font-weight: bold;">Mobile No:</span> ${phone_number || "N/A"}</p>
          <div style="margin: 10px 0;"></div>
          ${groupOrLoanHtml}
          <div class="line" style="margin: 5px 0;"></div>
          <table style="border-collapse: collapse; width: 100%; border: 1px solid #000; margin: 5px 0; font-size: 1.2em;">
            <tr><td style="padding: 5px; font-weight: bold;">Received Amount</td><td style="padding: 5px; text-align: right; font-weight: bold;">Rs.${amount || "0"}</td></tr>
          </table>
          <p><span style="font-weight: bold;">Mode:</span> ${pay_type || "N/A"}</br>
            ${txnLine}
            <span style="font-weight: bold;">Remaining Loan:</span> ${remainingLoan || "N/A"}</br>
            <span style="font-weight: bold;">Total:</span> Rs.${totalPaidAmount || "0"}</p>
          <div class="line"></div>
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.white }}>
      <View style={{ marginHorizontal: 22, marginTop: 12 }}>
        ...
        {/* TOP ACTION ROW: Connect + WhatsApp */}
    ...
    <Text style={styles.btnText}> Share</Text>
        ...
        >
          ...
          <Text style={[styles.textStyle, { fontSize: 14, fontWeight: "bold", marginBottom: 5 }]}>{groupOrLoanDisplay}</Text>
...
const styles = StyleSheet.create({
  ...
  printButtonOne: { backgroundColor: COLORS.third, padding: 20 },
  printButtonTwo: { backgroundColor: COLORS.third, padding: 20 },
