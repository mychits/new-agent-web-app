...
const LoanPrint = ({ route }) => {
    ...
    const { payInfo, agent, totalAmount, isLoanPayment } = route.params;

    const [isConnected, setIsConnected] = useState(false);
    const [isConnecting, setIsConnecting] = useState(false);

    // No need for useEffect hooks to fetch data anymore
    ...
    const handlePrint = () => {
        ...
        const groupOrLoan = isLoanPayment
            ? `Loan ID: ${payInfo?.payment_group_tickets[0].split('-')[1]}`
...
`;
    ...
    const handlePosPrint = async () => {
        const groupOrLoanHtml = isLoanPayment
            ? `<p style="margin: 0;">Loan ID: ${payInfo?.payment_group_tickets[0].split('-')[1]}</p>`
            : `<p style="margin: 0;">Group: ${payInfo?.group_id?.group_name || "N/A"}</p><p style="margin: 0;">Ticket: ${payInfo?.ticket || "N/A"}</p>`;
      ...
      <body>
        <div class="receipt">
          ...
          <div class="line"></div>
          <p align="center" style="font-weight:bold">${isLoanPayment ? 'Loan Receipt' : 'Receipt'}</p>
          <p>
